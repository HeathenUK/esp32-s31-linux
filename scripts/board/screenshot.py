...
CAPTURE = r'''
ADDR=$(dmesg | sed -n 's/.*scanout buffer [0-9]* bytes at 0x\([0-9a-f]*\).*/\1/p' | tail -1)
[ -z "$ADDR" ] && ADDR=$(dmesg | sed -n 's/.*scanout started.*fb=0x\([0-9a-f]*\).*/\1/p' | tail -1)
echo "ADDR $ADDR"
dd if=/dev/mem bs=1024 skip=$(( 0x$ADDR / 1024 )) count=750 2>/dev/null | gzip -9 > /tmp/fb.gz
echo BEGIN_B64
base64 /tmp/fb.gz
echo END_B64
'''
