			       ...
			       bool async, u32 src_bpp)
{
	...
	/*
	 * SAY SO WHEN THE RECTANGLE CANNOT BE SCALED EXACTLY.
	 *
	 * The factor comes from the rectangle, not the picture, and it is
	 * truncated to sixteenths. A caller that asks for an output the
	 * factor cannot produce does not get an error - it gets a band
	 * written at a slightly different scale, which reads as content at
	 * the wrong offset with the previous, correctly scaled copy still
	 * underneath. That cost a long hunt through xshim and lvdesk before
	 * anyone suspected the engine, because every individual frame looked
	 * plausible.
	 *
	 * So: correct the geometry to what the engine will actually produce -
	 * the implied output is never larger than the requested one, so the
	 * bounds check above still holds - and tell whoever asked. A caller
	 * that wants an exact result must align its rectangle to
	 * 16 / gcd(q, 16) on each axis.
	 */
	{
		u32 qw = sx_int * PPA_SRM_SCAL_FRAG_MAX + sx_frag;
		u32 qh = sy_int * PPA_SRM_SCAL_FRAG_MAX + sy_frag;
		u32 iw = bw * qw / PPA_SRM_SCAL_FRAG_MAX;
		u32 ih = bh * qh / PPA_SRM_SCAL_FRAG_MAX;

		if (iw != out_w || ih != out_h) {
			dev_warn_once(ppa->dev,
				      "srm: %ux%u -> %ux%u is not a whole number of sixteenths; the engine will write %ux%u. Align the source rectangle to 16/gcd(q,16).\n",
				      bw, bh, out_w, out_h, iw, ih);
			out_w = iw;
			out_h = ih;
		}
	}
