					...
					u32 n_bytes, int *xfer_done)
{
	...
	if ((dma_desc->status & HOST_DMA_STS_MASK) == HOST_DMA_STS_PKTERR) {
		...
		case DWC2_HC_XFER_XACT_ERR:
			/*
			 * THREE STRIKES, as the USB spec requires - not one.
			 *
			 * A transaction error is a normal, recoverable event on
			 * a real bus: the host is required to retry it up to
			 * three times before telling software the transfer
			 * failed (USB 2.0, 8.5.3.2). dwc2's non-DDMA path does
			 * exactly that, gating completion on
			 * `qtd->error_count >= 3` in dwc2_hc_xfer_done(). This
			 * descriptor-DMA path did not: the FIRST XactErr set
			 * -EPROTO and completed the URB.
			 *
			 * For an interrupt IN endpoint that is the difference
			 * between a hiccup and a dead keyboard. usbhid treats
			 * -EPROTO as an I/O error and backs off exponentially -
			 * 13 ms, 26 ms, 52 ms, doubling - leaving the endpoint
			 * unpolled, so every keystroke made in that window is
			 * never collected. Measured on this board: a wireless
			 * keyboard receiver that dropped whole sentences and
			 * needed a physical replug to recover, while a mouse on
			 * a different receiver was untouched.
			 *
			 * Returning 0 leaves the QTD queued, and a periodic
			 * endpoint is simply polled again next interval, which
			 * is the retry. Nothing else has to re-arm it.
			 */
			if (++qtd->error_count < 3) {
				dev_dbg(hsotg->dev,
					"XactErr %d/3 - retrying\n",
					qtd->error_count);
				return 0;
			}
			dev_err(hsotg->dev, "XactErr (3 of 3)\n");
			urb->status = -EPROTO;
			break;
		default:
			dev_err(hsotg->dev,
	...
	/*
	 * A descriptor that came back clean ends the run of errors. Without
	 * this the count is cumulative for the life of the QTD and the third
	 * error ever - however far apart - would still fail the transfer.
	 */
	qtd->error_count = 0;
