				  ...
				  bool scaled, u32 x1, u32 y1, u32 x2, u32 y2)
{
	...
	/*
	 * CPU vs PPA for a SCALED copy is decided on the OUTPUT size, not the
	 * source. CPU scaling costs one write per output pixel, so a full
	 * frame upscaled to fill the panel (a fullscreen game: 320x200 source
	 * is 128 KB, just under the source threshold, but 760x475 output is
	 * 722 KB) landed on the CPU at 22 fps where the PPA does it at 30.
	 * Gate on output bytes; a small damage rect still scales on the CPU,
	 * where the PPA's per-op setup would not pay off.
	 */
	if (scaled && lcd->scale_xmap && obj->vaddr) {
		size_t obytes = pl->render_w && pl->render_h ?
			(size_t)(x2 - x1) * pl->out_w / pl->render_w *
			((size_t)(y2 - y1) * pl->out_h / pl->render_h) * 2 :
			bytes;

		if (obytes < ESP32S31_PPA_MIN_BYTES) {
