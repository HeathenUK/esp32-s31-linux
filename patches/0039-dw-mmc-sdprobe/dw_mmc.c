 ...
 * Software queue.
 *
 * An SD request on this board costs ~8 ms, and almost none of that is the
 * card: 0 busy waits in 2617 calls, against ~200 us of actual bus time for
 * 4 KB at 4-bit/40 MHz. Pinning the CPU with a busy loop made reads 56%
 * *worse*, which cannot happen if the path is idling on hardware. Profiling
 * put ~0.9 ms/request into workqueue dispatch alone - process_scheduled_works,
 * __queue_work, bh_worker - for a path that is one interrupt and one
 * completion, with the scheduler machinery around it costing more again.
 *
 * That is what the MMC host software queue exists to remove: the core waits
 * for the previous request in mmc_blk_rw_wait() before issuing the next and
 * bounces completion through a workqueue, and HSQ instead keeps requests in
 * flight so the interrupt handler starts the next one without a context
 * switch. Upstream only sdhci-sprd wires it up; the mechanism is host-neutral.
 *
 * mmc_hsq_init() only publishes cqe_ops - the SD core turns it on during card
 * initialisation and announces "Host Software Queue enabled".
 *
 * It is wired up here but CONFIG_MMC_HSQ is OFF, because measuring it showed
 * the gain does not scale - it inverts. O_DIRECT reads, cache dropped, fresh
 * boot per arm:
 *
 *	request     no HSQ      HSQ
 *	   4K      2.66 ms   2.35 ms   -12%
 *	  16K      3.16 ms   3.01 ms    -5%
 *	  64K      5.31 ms   5.47 ms    +3%
 *	 256K     14.38 ms  18.12 ms   +26%
 *	1024K     55.00 ms 102.50 ms   +86%   (18.2 -> 9.8 MB/s)
 *
 * That is consistent with what it does: it removes a fixed ~0.3 ms per
 * request, which dominates a 4 kB read and is noise in a 1 MB one, while its
 * in-flight limiting gets in the way of large transfers. Swap-in is 4 kB and
 * benefits, but file and executable paging read through readahead at 128 kB
 * and up, and the desktop does far more of that. Matched desktop arms agreed:
 * repaint mean 20.2 vs 25.0 ms, process launch 380 vs 420 ms, both favouring
 * HSQ off.
 *
 * Kept rather than deleted so the option can be re-measured if the workload
 * changes - the integration is correct and compiles to nothing when off.
 *
 * Worth recording separately: a 4 kB O_DIRECT read costs 2.65 ms on an idle
 * system, not the ~8 ms this board's notes quote. That figure came from a
 * loaded machine and is contention, not the storage path.
 */
#if IS_ENABLED(CONFIG_MMC_HSQ)
#include "mmc_hsq.h"

static int dw_mci_hsq_init(struct dw_mci *host, struct mmc_host *mmc)
{
	struct mmc_hsq *hsq;
	...
	return mmc_hsq_init(hsq, mmc);
}

static bool dw_mci_hsq_finalize(struct mmc_host *mmc, struct mmc_request *mrq)
{
	return mmc_hsq_finalize_request(mmc, mrq);
}
#else
static int dw_mci_hsq_init(struct dw_mci *host, struct mmc_host *mmc)
{
...
/*
 * The lost-interrupt poll is a workaround, and workarounds should be
 * measurable. Toggling it at runtime makes it possible to A/B its cost on one
 * boot instead of reflashing between builds:
 *
 *     echo 0 > /sys/module/dw_mmc/parameters/lost_irq_poll
 *
 * Defaults OFF as of 2026-08-22, because it was A/B'd and does nothing. Same
 * boot, 200 random 4k O_DIRECT reads each way:
 *
 *     poll on    p50 3.807 ms   2.91 interrupts per request
 *     poll off   p50 3.737 ms   3.00 interrupts per request
 *
 * Interrupts are not being lost - the count per request is if anything higher
 * without the poll - so this was arming an hrtimer per request to recover
 * something that never happens. Left in place rather than deleted because it
 * is the instrument that proved the point: with the poll on, MINTSTS is read
 * every 0.5 ms, so any status the controller had raised would be observed
 * within that. CMD_DONE still takes 2.07 ms to appear, which is how we know
 * the delay is the controller raising it and not Linux delivering it.
 */
static bool lost_irq_poll;	/* measured worthless; see below */

/*
 * Card-clock gating. CLKENA bit 16 stops the card clock whenever the bus is
 * idle, so every command has to restart a stopped clock domain and wait for the
 * CIU to resynchronise before it can be sent.
 *
 * That is worth testing here because the command phase, not the data phase, is
 * where this board's per-request cost lives: of a 3.78 ms random 4k read,
 * 1.53 ms is between writing the command register and CMD_DONE, while the data
 * phase measures 241 us - about right for 4 KB at 20 MB/s. A CMD17 is roughly
 * 2.4 us on the wire, so 1.53 ms is not the card answering.
 *
 * Measured, and it is NOT the cause - left on, because gating saves power and
 * costs nothing detectable. Same card, random 4k O_DIRECT reads:
 *
 *     CLKENA 0x00010001 (gating on)   p50 3.778 ms
 *     CLKENA 0x00000001 (gating off)  p50 3.781 ms
 *
 * Kept as a runtime knob so the next person does not have to rebuild to
 * re-check it.
 */
static bool low_pwr = true;
module_param(low_pwr, bool, 0644);
MODULE_PARM_DESC(low_pwr, "gate the card clock when the bus is idle");
module_param(lost_irq_poll, bool, 0644);
MODULE_PARM_DESC(lost_irq_poll,
		 "poll for interrupts the controller loses (ESP32-S31)");

static bool dw_mci_lost_irq_poll(struct dw_mci *host)
{
	return lost_irq_poll && (host->quirks & DW_MMC_QUIRK_LOST_IRQ_POLL);
}
...
/*
 * Per-request timing for the ~10 ms fixed cost every SD request pays on this
 * board, whatever its size. It is charged to CPU, not to waiting: CoreMark
 * falls from 916 to 151 under 4k reads moving 0.4 MB/s, and to 136 under 1M
 * reads moving 12.7 MB/s - the same ~85% of the hart for 32x less data.
 *
 * Ruled out already, each by measurement: cache maintenance (1% of wall time),
 * the lost-interrupt poll (changes neither throughput nor CPU, and interrupts
 * arrive at ~3 per request so the IRQ path works), and queue depth (1 to 8
 * concurrent readers moves 0.401 to 0.462 MB/s).
 *
 * That leaves the busy poll below, which runs before every data command. Three
 * plausible explanations have already turned out to be wrong, so this counts
 * rather than assumes. Note it cannot simply be made to sleep instead: the call
 * runs under host->lock from __dw_mci_start_request().
 *
 *     /sys/kernel/debug/mmc0/busy_wait
 */
/*
 * Where does the ~10 ms every request costs actually go?
 *
 * Eight explanations have been eliminated by measurement - the busy poll below
 * (exactly zero), the lost-interrupt poll (~7%), cache maintenance (1%), queue
 * depth, controller configuration, context switches, interrupt count and MMIO
 * latency - so this stops guessing and times the request itself, split at the
 * points the hardware tells us about:
 *
 *   issue -> cmd_done   the command going out and being acknowledged
 *   cmd_done -> data    the card actually moving data
 *   data -> end         software finishing the request
 *
 *     /sys/kernel/debug/mmc0/req_timing
 */
static u64 dw_mci_t_issue, dw_mci_t_cmd, dw_mci_t_data, dw_mci_t_prev_end;
static atomic64_t dw_mci_req_count;
static atomic64_t dw_mci_ns_issue_cmd, dw_mci_ns_cmd_data, dw_mci_ns_data_end;
static atomic64_t dw_mci_ns_total, dw_mci_ns_worst;

/*
 * SDPROBE, 2026-09-12. Attributing the ~1.8 ms fixed cost of a request.
 *
 * sdlat(1) in userspace measures a 4 KiB O_DIRECT random read at min 1.91 ms
 * and a size sweep extrapolates a fixed cost of ~1.8 ms with a marginal rate
 * of ~49 MB/s, so the fixed part is not transfer time. The three debugfs
 * files this file already defines (req_timing, cmd_probe, busy_wait) were
 * never registered and their counters were never written, so nothing here
 * has ever attributed that cost - see dw_mci_init_debugfs().
 *
 * Two candidates are measured, chosen because both are size-independent and
 * both sit OUTSIDE the old timing window (t_issue is stamped after the CMD
 * write, while dw_mci_submit_data() runs before it):
 *
 *   prep  - the whole of dw_mci_submit_data(): dma_map_sg, descriptor
 *           preparation, and the unconditional DMA reset below.
 *   reset - dw_mci_ctrl_reset(), which spins on a CTRL bit that clears in
 *           the CIU clock domain, on EVERY request.
 *
 * ktime_get_ns() is NOT free on this board - the systimer does an MMIO
 * write, a polled read and two more reads per call - so its own cost is
 * measured at probe and reported alongside. An instrument that costs as
 * much as the thing it measures has produced two wrong answers here
 * already this year.
 */
static atomic64_t dw_mci_ns_prep, dw_mci_n_prep;
static atomic64_t dw_mci_ns_reset, dw_mci_n_reset;
static u64 dw_mci_ns_ktime_self;
static struct dw_mci *dw_mci_probe_host;

/*
 * The gap between one request finishing and the next being issued. The first
 * measurement put only ~2 ms of an ~11 ms request inside the driver, so most of
 * the cost is here - in the block layer, the mmc core, or waking the submitter.
 */
static atomic64_t dw_mci_ns_gap, dw_mci_gap_count;

/*
 * Which of the two remaining explanations for the 2.07 ms between writing the
 * command register and seeing CMD_DONE is true.
 *
 * It is already known not to be Linux's interrupt delivery: with the lost-irq
 * poll enabled MINTSTS is read every 0.5 ms, so anything the controller had
 * raised would be observed within that, and CMD_DONE still takes 2.07 ms. The
 * controller is not raising it until then. That leaves:
 *
 *   cmd_alone      CMD_DONE observed with DATA_OVER still clear - the command
 *                  phase itself is slow, so look at the CIU and its clock.
 *   cmd_with_data  both pending in the same read - the controller completed
 *                  the whole transaction before telling us anything, so the
 *                  2 ms is the card's access time and no driver change fixes
 *                  it; a faster card would.
 *
 * It is cmd_alone, and it is NOT the card. That was tested rather than
 * assumed: swapping the 8 GB SDHC card for a SanDisk SDXC one, same tool, same
 * offsets, random O_DIRECT reads:
 *
 *     4k    3.78 -> 3.24 ms
 *     32k   4.31 -> 3.84 ms
 *     64k   4.98 -> 4.26 ms
 *
 * 14%, not the order of magnitude a card-bound cost would give, and cmd_alone
 * still averages ~2.3 ms. So the command phase is the controller's, and the
 * DAT0-busy theory below is dead. What makes it strange is that commands and
 * data share one clock pin: the data phase measures ~240 us for 4 KB, which is
 * 40 MHz on a 4-bit bus, while a 48-bit command on that same clock takes 2 ms.
 *
 * Next instrument, when someone picks this up: time a command with no data
 * (CMD13) through MMC_IOC_CMD. If that is also ~2 ms the CIU command state
 * machine is the whole story; if it is fast, something specific to data
 * commands is stalling before the command goes out.
 *
 *     /sys/kernel/debug/mmc0/cmd_probe
 */
static atomic64_t dw_mci_cmd_alone, dw_mci_cmd_with_data;
static atomic64_t dw_mci_ns_alone, dw_mci_ns_with_data;
...
static atomic64_t dw_mci_busy_waited;	/* calls where BUSY was actually set */
static atomic64_t dw_mci_busy_ns;
static atomic64_t dw_mci_busy_max_ns;

/*
 * Read out through a module parameter, not debugfs. DIAG=1 puts the kernel
 * 16 KB past the linux partition, and debugfs also costs 1.14 MB of RAM -
 * which would perturb the page-cache pressure this is being used to study.
 * lost_irq_poll already proves the mechanism in this driver.
 */
static int dw_mci_probe_get(char *buf, const struct kernel_param *kp)
{
	u64 np = atomic64_read(&dw_mci_n_prep);
	u64 nr = atomic64_read(&dw_mci_n_reset);
	struct dw_mci *h = dw_mci_probe_host;

	u64 ng = atomic64_read(&dw_mci_gap_count);
	u64 nwd = atomic64_read(&dw_mci_cmd_with_data);
	u64 na = atomic64_read(&dw_mci_cmd_alone);
	...
	return scnprintf(buf, PAGE_SIZE,
			 "ktime_self_ns %llu\n"
			 "prep     n %llu avg_ns %llu\n"
			 "reset    n %llu avg_ns %llu\n"
			 "cmd_data n %llu avg_ns %llu\n"
			 "cmd_alone n %llu avg_ns %llu\n"
			 "gap      n %llu avg_ns %llu\n"
			 "busy     n %llu waited %llu avg_ns %llu\n"
			 ...
			 nr, nr ? div64_u64((u64)atomic64_read(&dw_mci_ns_reset), nr) : 0,
			 nwd, nwd ? div64_u64((u64)atomic64_read(&dw_mci_ns_with_data), nwd) : 0,
			 na, na ? div64_u64((u64)atomic64_read(&dw_mci_ns_alone), na) : 0,
			 ng, ng ? div64_u64((u64)atomic64_read(&dw_mci_ns_gap), ng) : 0,
			 nb, (u64)atomic64_read(&dw_mci_busy_waited),
			 nb ? div64_u64((u64)atomic64_read(&dw_mci_busy_ns), nb) : 0,
			 ...
			 h ? h->mmc->ios.bus_width : -1);
}

static const struct kernel_param_ops dw_mci_probe_ops = {
	.get = dw_mci_probe_get,
};
module_param_cb(sdprobe, &dw_mci_probe_ops, NULL, 0444);

static void dw_mci_busy_account(u64 ns, bool waited)
{
	u64 old;

	atomic64_inc(&dw_mci_busy_calls);
	if (!waited)
		return;

	atomic64_inc(&dw_mci_busy_waited);
	atomic64_add(ns, &dw_mci_busy_ns);

	do {
		old = atomic64_read(&dw_mci_busy_max_ns);
		if (ns <= old)
			break;
	} while (atomic64_cmpxchg(&dw_mci_busy_max_ns, old, ns) != old);
}
...
static int dw_mci_req_timing_show(struct seq_file *s, void *v)
{
	u64 n = atomic64_read(&dw_mci_req_count);

	seq_printf(s, "requests=%llu worst_ns=%llu\n", n,
		   atomic64_read(&dw_mci_ns_worst));
	if (!n)
		return 0;
	/* rv32 has no 64-bit divide; __udivdi3 is libgcc and not linked here. */
	{
		u64 g = atomic64_read(&dw_mci_gap_count);

		seq_printf(s, "gap_between_requests_ns=%llu (n=%llu)\n",
			   g ? div64_u64(atomic64_read(&dw_mci_ns_gap), g) : 0, g);
	}
	seq_printf(s, "issue_to_cmd_ns=%llu\ncmd_to_data_ns=%llu\ndata_to_end_ns=%llu\ntotal_ns=%llu\n",
		   div64_u64(atomic64_read(&dw_mci_ns_issue_cmd), n),
		   div64_u64(atomic64_read(&dw_mci_ns_cmd_data), n),
		   div64_u64(atomic64_read(&dw_mci_ns_data_end), n),
		   div64_u64(atomic64_read(&dw_mci_ns_total), n));
	return 0;
}
DEFINE_SHOW_ATTRIBUTE(dw_mci_req_timing);

static int dw_mci_cmd_probe_show(struct seq_file *s, void *v)
{
	u64 a = atomic64_read(&dw_mci_cmd_alone);
	u64 d = atomic64_read(&dw_mci_cmd_with_data);
	int i;

	seq_printf(s, "cmd_alone=%llu avg_ns=%llu\n", a,
		   a ? div64_u64(atomic64_read(&dw_mci_ns_alone), a) : 0);
	seq_printf(s, "cmd_with_data=%llu avg_ns=%llu\n", d,
		   d ? div64_u64(atomic64_read(&dw_mci_ns_with_data), d) : 0);
	for (i = 0; i < 4; i++)
		seq_printf(s, "first_status[%d]=0x%08x\n", i,
			   dw_mci_first_status[i]);
	return 0;
}
DEFINE_SHOW_ATTRIBUTE(dw_mci_cmd_probe);

static int dw_mci_busy_wait_show(struct seq_file *s, void *v)
{
	u64 calls = atomic64_read(&dw_mci_busy_calls);
	u64 waited = atomic64_read(&dw_mci_busy_waited);
	u64 ns = atomic64_read(&dw_mci_busy_ns);

	seq_printf(s, "calls=%llu waited=%llu total_ns=%llu max_ns=%llu\n",
		   calls, waited, ns,
		   atomic64_read(&dw_mci_busy_max_ns));
	/* rv32 has no 64-bit divide; __udivdi3 is libgcc and not linked here. */
	seq_printf(s, "ns_per_call=%llu ns_per_wait=%llu\n",
		   calls ? div64_u64(ns, calls) : 0,
		   waited ? div64_u64(ns, waited) : 0);
	return 0;
}
DEFINE_SHOW_ATTRIBUTE(dw_mci_busy_wait);
...
static bool dw_mci_ctrl_reset(struct dw_mci *host, u32 reset)
{
	...
	atomic64_add(ktime_get_ns() - t0, &dw_mci_ns_reset);
	atomic64_inc(&dw_mci_n_reset);
	return true;
}
				 ...
				 struct mmc_command *cmd, u32 cmd_flags)
{
	...
	/*
	 * EVERY command, not just the ones carrying data. The timing window
	 * below is gated on DAT_EXP, so any status poll or stop command the
	 * mmc core issues around a transfer is invisible to it - and a
	 * no-data command was measured at 0.845 ms on this board. If the
	 * ratio of this counter to the data-command count is not ~1, the
	 * missing ~1.6 ms of a 4 KiB read is extra commands.
	 */
	atomic64_inc(&dw_mci_n_cmd_all);
	dw_mci_last_opcode = cmd->opcode;
	if (cmd_flags & SDMMC_CMD_DAT_EXP) {
		dw_mci_t_issue = ktime_get_ns();
		if (dw_mci_t_prev_end) {
			atomic64_add(dw_mci_t_issue - dw_mci_t_prev_end,
				     &dw_mci_ns_gap);
			atomic64_inc(&dw_mci_gap_count);
		}
		dw_mci_t_cmd = 0;
		dw_mci_t_data = 0;
	}
...
/*
 * 7.1 merged prepare_desc64/32 into one function. Three BSP hunks then had
 * nowhere to land, and patching the merged version by hand produced a driver
 * that was correct but interrupted once per descriptor - 242 IRQs/MB against
 * 15, and 2 MB/s against 12. Carry the 6.12 implementations verbatim instead
 * and dispatch to them; they are the ones proven on this controller.
 */
static inline int dw_mci_prepare_desc(struct dw_mci *host, struct mmc_data *data,
				      unsigned int sg_len, bool is_64bit)
{
	if (is_64bit)
		return dw_mci_prepare_desc64(host, data, sg_len);
	return dw_mci_prepare_desc32(host, data, sg_len);
}
...
static void dw_mci_setup_bus(struct dw_mci *host, bool force_clkinit)
{
	...
	if (!clock) {
		...
		if (low_pwr && !test_bit(DW_MMC_CARD_NO_LOW_PWR, &host->flags))
			clk_en_a |= SDMMC_CLKEN_LOW_PWR;
		mci_writel(host, CLKENA, clk_en_a);
...
static void dw_mci_start_request(struct dw_mci *host, struct mmc_command *cmd)
{
	...
	if (data) {
		...
		wmb(); /* drain writebuffer */
		atomic64_add(ktime_get_ns() - t0, &dw_mci_ns_prep);
		atomic64_inc(&dw_mci_n_prep);
	}
...
static void dw_mci_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (!dw_mci_get_cd(mmc)) {
		...
		if (!dw_mci_hsq_finalize(mmc, mrq))
...
static irqreturn_t dw_mci_interrupt_once(int irq, void *dev_id)
{
	...
	if (pending) {
		/*
		 * Stamp both completions here, from one read of the pending
		 * mask, before any branch below runs.
		 *
		 * They used to be stamped inside their own branches, and the
		 * DATA_OVER branch runs first, so when the controller raised
		 * both together t_data was taken BEFORE t_cmd. The accumulator
		 * guards on t_data > t_cmd, so those requests were dropped from
		 * cmd_to_data entirely and the average described only the subset
		 * that arrived separately - exactly the subset where the split is
		 * least interesting. cmd_to_data == 0 now honestly means "both
		 * arrived in the same interrupt".
		 */
		if (dw_mci_t_issue) {
			u64 now = ktime_get_ns();

			if ((pending & SDMMC_INT_CMD_DONE) && !dw_mci_t_cmd) {
				dw_mci_t_cmd = now;
				/*
				 * THE MISSING WRITER. The stamps were taken
				 * and thrown away - nothing accumulated them,
				 * so req_timing could only ever print zero.
				 * This is the interval that matters: the CMD
				 * register write to the controller saying the
				 * command is done.
				 */
				atomic64_add(now - dw_mci_t_issue,
					     &dw_mci_ns_issue_cmd);
				atomic64_inc(&dw_mci_req_count);
			}
			if ((pending & SDMMC_INT_DATA_OVER) && !dw_mci_t_data) {
				dw_mci_t_data = now;
				/* command done (or issue, if they came in the
				 * same interrupt) to the last data byte. */
				atomic64_add(now - (dw_mci_t_cmd ? dw_mci_t_cmd
							        : dw_mci_t_issue),
					     &dw_mci_ns_cmd_data);
				atomic64_inc(&dw_mci_ns_data_end);
			}
		}
		...
		if (pending & SDMMC_INT_CMD_DONE) {
			if (dw_mci_t_issue && !dw_mci_t_cmd) {
				u64 dt;

				dw_mci_t_cmd = ktime_get_ns();
				dt = dw_mci_t_cmd - dw_mci_t_issue;
				if (pending & SDMMC_INT_DATA_OVER) {
					atomic64_inc(&dw_mci_cmd_with_data);
					atomic64_add(dt, &dw_mci_ns_with_data);
				} else {
					atomic64_inc(&dw_mci_cmd_alone);
					atomic64_add(dt, &dw_mci_ns_alone);
				}
				{
					int i = atomic_inc_return(&dw_mci_status_slot) - 1;

					if (i < 4)
						dw_mci_first_status[i] = pending;
				}
			}
			spin_lock(&host->irq_lock);
...
static int dw_mci_init_host(struct dw_mci *host)
{
	...
	/* Must precede mmc_add_host(): the core enables it during card init. */
...
#endif
	/*
	 * OUTSIDE the DEBUG_FS guard. The first version of this sat inside it
	 * and the shipping kernel builds DIAG=0, so the host pointer was never
	 * set and the readout printed zeros - which is exactly the class of
	 * silent-instrument failure this whole exercise is about.
	 *
	 * Measure the instrument before trusting anything it says: 64
	 * back-to-back reads, reported per pair.
	 */
	{
		u64 t0, t1 = 0;
		int i;

		dw_mci_probe_host = host;
		t0 = ktime_get_ns();
		for (i = 0; i < 64; i++)
			t1 = ktime_get_ns();
		dw_mci_ns_ktime_self = (t1 - t0) >> 6;
	}
