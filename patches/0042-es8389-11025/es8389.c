 ...
 *
 * Backported from mainline. The only change is the two DAPM helpers, which
 * were named snd_soc_dapm_kcontrol_{component,dapm}() in this kernel and
 * gained a "to_" infix later; the signatures are otherwise identical.
 *
...
static unsigned int mainclk_src = 0x40;
...
static const struct _coeff_div  coeff_div[] = {
	...
	/*
	 * 11025 and 22050 Hz at ratio 32, for this board. NOT vendor rows.
	 *
	 * This board has no MCLK line (esp32s31_generic.dts, "No mclk-fs"),
	 * so hw_params derives the codec clock from the bit clock:
	 * channels * width * rate = 32 * rate for S16 stereo. The 11025 row
	 * further down was written for MCLK = 256 * rate = 2822400 and could
	 * therefore never match here: every 11025 stream since it landed
	 * logged "Clock coefficients do not match", left the clock dividers at
	 * their idle values, and played as a loud hiss with the audio faint and
	 * muffled - which is what Doom (prboom mixes at 11025) sounded like for
	 * days while a 48000 tone through the same route sounded right.
	 * Diagnosed 2026-09-19 from dmesg plus an I2C register dump during the
	 * stream. Cloned from the 44100 ratio-32 row above: 44100 and 48000
	 * share their row byte for byte, so these values are a family setting,
	 * not a per-rate calibration. VERIFY BY EAR.
	 */
	{32, 352800, 11025, 0x00, 0x45, 0xA4, 0xD0, 0x10, 0xD1, 0x80, 0x00, 0x00, 0x1F, 0x7F, 0xBF, 0xC0, 0x7F, 0x7F, 0x00, 0x12, 0x00, 0x35, 0x91, 0x28, 2, 2},
	{32, 705600, 22050, 0x00, 0x45, 0xA4, 0xD0, 0x10, 0xD1, 0x80, 0x00, 0x00, 0x1F, 0x7F, 0xBF, 0xC0, 0x7F, 0x7F, 0x00, 0x12, 0x00, 0x35, 0x91, 0x28, 2, 2},
	...
	/*
	 * 11025 and 22050 at ratio 64: with dac_ref=Y (this board) hw_params
	 * derives the clock as width * rate * 4 = 64 * rate for S16, so THESE
	 * are the rows that match here. Cloned from the 44100 ratio-64 row.
	 * The ratio-32 rows below cover dac_ref=N. VERIFY BY EAR.
	 */
	{64, 705600, 11025, 0x00, 0x51, 0x00, 0xC0, 0x10, 0xC1, 0x80, 0x00, 0x00, 0x1F, 0x7F, 0xBF, 0xC0, 0x7F, 0x7F, 0x00, 0x12, 0x00, 0x35, 0x91, 0x28, 2, 2},
	{64, 1411200, 22050, 0x00, 0x51, 0x00, 0xC0, 0x10, 0xC1, 0x80, 0x00, 0x00, 0x1F, 0x7F, 0xBF, 0xC0, 0x7F, 0x7F, 0x00, 0x12, 0x00, 0x35, 0x91, 0x28, 2, 2},
	...
	/*
	 * 11025 Hz, added for this board. NOT a vendor row.
	 *
	 * Doom's sound effects are 11025 Hz and prboom's mixer is a
	 * per-output-sample loop, so its cost scales linearly with the
	 * output rate - 44100 is four times the work of 11025, and at
	 * 11025 its resampling step is exactly 1.0 so the interpolation
	 * degenerates to a copy. The vendor table populates the 8 kHz and
	 * 48 kHz families heavily and gives the 44.1 kHz family only
	 * ratios 32 to 512, with no 11025 or 22050 anywhere. That is a
	 * characterisation gap, not a silicon limit: the chip already runs
	 * this ratio and this clock.
	 *
	 * Ratio 256 and MCLK 2822400 are what our I2S driver will actually
	 * ask for - it fixes MCLK at rate * 256
	 * (ESP32S31_I2S_MCLK_MULTIPLE) - and get_coeff() matches on rate
	 * and mclk exactly. 2822400 Hz is already in this table, as 44100
	 * at ratio 64, so the clock is known good.
	 *
	 * The registers are cloned from the 44100 ratio-256 row above.
	 * That is defensible because 44100 and 48000 share that row
	 * BYTE FOR BYTE despite being 8%% apart, so these values are not a
	 * fine per-rate calibration. It is still a guess in one respect:
	 * the last three columns track the rate across families
	 * (0x09/0x19/0x07 at 8 kHz, 0x12/0x31/0x0E at 16 kHz,
	 * 0x35/0x91/0x28 at 44.1 and 48 kHz) and are plausibly a filter or
	 * oversampling setting. If 11025 sounds muffled, aliased or
	 * wrong-pitched rather than silent, that triple is the thing to
	 * scale. VERIFY BY EAR - this project has had instruments call a
	 * pure-noise stream working four times.
	 */
	{256, 2822400, 11025, 0x01, 0x41, 0x04, 0xD0, 0x10, 0xD1, 0x80, 0x40, 0x00, 0x1F, 0x7F, 0xBF, 0xC0, 0x7F, 0x7F, 0x00, 0x12, 0x00, 0x35, 0x91, 0x28, 2, 2},
			...
			struct snd_soc_dai *dai)
{
	...
	if (es8389->mclk_src == ES8389_SCLK_PIN) {
		/*
		 * Register 0x02 bits 7:6 select the clock for the state machine
		 * and filter logic, and 01 means SCLK. Updating bit 6 to
		 * mclk_src (0) selects 00, the MCLK pad - the opposite of what
		 * was asked for - and doing it here undoes whatever init set,
...
static void es8389_init(struct snd_soc_component *component)
{
	...
	/*
	 * Register 0x02 bits 7:6 pick the clock for the state machine and the
	 * filter logic: 00 takes it from the MCLK pad, 01 from SCLK. Boards
	 * that route no MCLK to the codec need 01, and everest,mclk-src cannot
	 * express that - it is applied to register 0x01 bit 6, which the
	 * datasheet documents as LRCK tristate. Overridable while that is
	 * being sorted out.
	 */
	regmap_write(es8389->regmap, ES8389_MASTER_CLK, mainclk_src);
