			       ...
			       bool async)
{
	 ...
	 */
	if (!trusted &&
	    (!esp32s31_ppa_in_range(ppa, src_addr, (size_t)src_w * src_h * 2) ||
	...
	/* Source: the whole picture as a single block. */
	esp32s31_ppa_desc_rgb565(ppa, DMA2D_DESC_SRC, src_addr, src_w, src_h,
			  bw, bh, sx, sy);
	/*
	...
	/* RGB565 in, RGB565 out: no colour conversion. */
	writel((PPA_SRM_CM_RGB565 << PPA_SRM_RX_CM_S) |
				  ...
				  u32 ow, u32 oh)
{
	...
	return esp32s31_ppa_srm_ex(ppa, src, src_w, src_h, dst, dst_w, dst_h,
				   sx, sy, bw, bh, dx, dy, ow, oh, false, true);
}
