...
struct esp32s31_lcd {
	/*
	 * gem_create_object() is handed only (device, size) - not the caller's
	 * intent - so the cached/write-combine choice has to reach it out of
	 * band. The ioctl sets this under gem_alloc_lock across its one call
	 * into the helper, which is the whole extent of the window.
	 */
	struct mutex gem_alloc_lock;
	bool gem_alloc_cached;
...
static int esp32s31_lcd_alloc_scanout(struct esp32s31_lcd *lcd)
{
	...
	lcd->scan_cpu = dma_alloc_coherent(lcd->drm.dev, size, &lcd->scan_phys,
					   GFP_KERNEL);
	if (!lcd->scan_cpu)
		return -ENOMEM;
				     ...
				     struct drm_plane_state *old_state)
{
	...
	if (esp32s31_lcd_composite(lcd)) {
		...
		if (damaged && old_state->fb == state->fb) {
			...
			drm_atomic_helper_damage_iter_init(&iter, old_state, state);
			drm_atomic_for_each_plane_damage(&iter, &clip) {
				u32 x1 = ALIGN_DOWN(max(clip.x1, 0), 32);
				u32 y1 = ALIGN_DOWN(max(clip.y1, 0), 32);
				u32 x2 = min_t(u32, clip.x2, fb->width);
				u32 y2 = min_t(u32, clip.y2, fb->height);
				  ...
				  bool scaled, u32 x1, u32 y1, u32 x2, u32 y2)
{
	...
	if (scaled && lcd->scale_xmap && obj->vaddr &&
	    bytes < ESP32S31_PPA_MIN_BYTES) {
		esp32s31_lcd_scale_rect_cpu(lcd, obj->vaddr, fb->pitches[0],
					    pl, x1, y1, x2, y2);
		lcd->dbg_path_cpuscale++;
		return;
	}

	if (!scaled && bytes < ESP32S31_PPA_MIN_BYTES && obj->vaddr) {
	...
	lcd->dbg_path_ppa++;
	ret = esp32s31_ppa_scale_rect(obj->dma_addr, fb->width, fb->height,
				      lcd->scan_phys, lcd->native.hdisplay,
				      lcd->native.vdisplay,
				      x1, y1, x2 - x1, y2 - y1,
				       ...
				       struct drm_file *file)
{
	 ...
	 *
	 * DO NOT gate these on map_noncoherent on the theory that a
	 * write-combine buffer has no cache lines to sync. It has them here:
	 * dma_alloc_wc on this SoC hands back CACHED memory, exactly as
	 * dma_alloc_coherent does (see the scanout allocation) - only the
	 * USERSPACE mapping is write-combine. Skipping the syncs was tried
	 * 2026-09-06 to save 192 KB of cache maintenance per frame at 320x200,
	 * and put horizontal bands of stale pixels through the window, visible
	 * on the physical panel. That maintenance is real work the hardware
	 * path genuinely owes, and it is part of why the PPA only breaks even
	 * against the CPU loop at this size.
	 */
...
/*
 * Honour the pending cached/write-combine choice for this allocation.
 *
 * __drm_gem_dma_create() only forces map_noncoherent off for dma-buf IMPORTS,
 * so whatever is set here survives for buffers we create ourselves - including
 * ones later exported over PRIME, which map with the same attributes.
 */
static struct drm_gem_object *esp32s31_gem_create_object(struct drm_device *drm,
							 size_t size)
{
	struct esp32s31_lcd *lcd = to_esp32s31_lcd(drm);
	struct drm_gem_dma_object *obj;
	...
	obj->map_noncoherent = lcd->gem_alloc_cached;
	return &obj->base;
}

static int esp32s31_lcd_gem_create_ioctl(struct drm_device *drm, void *data,
					 struct drm_file *file)
{
	struct drm_esp32s31_gem_create *args = data;
	struct esp32s31_lcd *lcd = to_esp32s31_lcd(drm);
	struct drm_gem_dma_object *obj;
	u32 pitch, handle;
	size_t size;
	...
	if (!args->w || !args->h)
		return -EINVAL;
	if (args->bpp != 8 && args->bpp != 16 && args->bpp != 32)
		return -EINVAL;
	if (args->flags & ~DRM_ESP32S31_GEM_CACHED)
		return -EINVAL;

	/*
	 * Tightly packed, exactly as MODE_CREATE_DUMB lays it out. Every
	 * consumer of these buffers - the shim, the CLUT expander - indexes
	 * them as w*h with no stride, so padding rows here would put the
	 * hardware and the CPU on different pixels for any width that is not
	 * a multiple of four.
	 */
	pitch = args->w * (args->bpp / 8);
	size = (size_t)pitch * args->h;
	if (!size || size > SZ_16M)
		return -EINVAL;

	mutex_lock(&lcd->gem_alloc_lock);
	lcd->gem_alloc_cached = !!(args->flags & DRM_ESP32S31_GEM_CACHED);
	obj = drm_gem_dma_create(drm, size);
	lcd->gem_alloc_cached = false;
	mutex_unlock(&lcd->gem_alloc_lock);

	if (IS_ERR(obj))
		return PTR_ERR(obj);

	ret = drm_gem_handle_create(file, &obj->base, &handle);
	drm_gem_object_put(&obj->base);	/* the handle holds the reference */
	if (ret)
		return ret;
...
static const struct drm_ioctl_desc esp32s31_lcd_ioctls[] = {
	DRM_IOCTL_DEF_DRV(ESP32S31_PPA_BLEND, esp32s31_lcd_ppa_blend_ioctl,
			  DRM_RENDER_ALLOW),
	DRM_IOCTL_DEF_DRV(ESP32S31_GEM_CREATE, esp32s31_lcd_gem_create_ioctl,
			  DRM_RENDER_ALLOW),
...
static const struct drm_driver esp32s31_lcd_driver = {
	...
	.gem_create_object	= esp32s31_gem_create_object,
...
static int esp32s31_lcd_updates_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "damage: last=%ux%u+%u+%u hist=%u,%u,%u,%u,%u\n",
		   ...
		   lcd->dbg_dmg_hist[4]);
	seq_printf(m, "path: rects=%u full=%u cpu=%u cpuscale=%u ppa=%u\n",
		   lcd->dbg_path_rects, lcd->dbg_path_full, lcd->dbg_path_cpu,
		   lcd->dbg_path_cpuscale, lcd->dbg_path_ppa);
