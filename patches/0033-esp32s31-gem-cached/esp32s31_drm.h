...
/*
 * Create a GEM buffer and say how the CPU should map it.
 *
 * MODE_CREATE_DUMB always gives write-combine, which is the right default for
 * something the CPU only ever writes - a scanout buffer - and the wrong one
 * for a surface it also READS. An 8-bit game surface is written by the client
 * and read by whoever expands the palette; the 16-bit result is read by the
 * compositor every frame. Uncached reads make both of those far more
 * expensive than the hardware expansion saves, and there is no way to ask
 * MODE_CREATE_DUMB for anything else.
 *
 * CACHED asks for dma_alloc_noncoherent() and an ordinary cached mapping. The
 * PPA ioctls already sync both buffers around the transfer, so a cached buffer
...
struct drm_esp32s31_gem_create {
	__u32 w;
	__u32 h;
	__u32 bpp;			/* 8, 16 or 32 */
	__u32 flags;			/* DRM_ESP32S31_GEM_CACHED */
	__u32 handle;			/* out */
	__u32 pitch;			/* out */
	__u64 size;			/* out */
};
...
#define DRM_IOCTL_ESP32S31_GEM_CREATE	DRM_IOWR(DRM_COMMAND_BASE + \
					DRM_ESP32S31_GEM_CREATE, \
					struct drm_esp32s31_gem_create)
